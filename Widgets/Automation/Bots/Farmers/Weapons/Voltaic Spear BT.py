...
def _summoning_stone() -> BehaviorTree:
    def _build(_node: BehaviorTree.Node) -> BehaviorTree:
        ...
        # Each receiver owns its inventory selection and summon/sickness checks.
        # Include the leader in the same dispatch; do not also consume locally.
        return BTShared.SendAndWait(
            command=SharedCommandType.UseSummoningStone,
            recipients=_inventory_recipient_emails(),
            include_self=True,
            refs_blackboard_key='voltaic_summoning_stone_refs',
            timeout_ms=10_000,
            poll_interval_ms=100,
            log=True,
        )
