...
def on_enable():
    ...
    # The quest log window is always visible now - no hotkey toggle.
...
def main():
    ...
    # --- quest log window: always drawn ---
    # The ShowOnlyInParty / ShowOnlyOnLeader settings gate the map overlays
    # only; the log itself is unconditional.
    if not Map.WorldMap.IsWindowOpen():
        UI.draw_log(quest_cache.quest_data, accounts)

    # --- minimap / mission map overlays: still gated ---
...
def tooltip():
    ...
    PyImGui.bullet_text("The quest log window is always shown.")
    PyImGui.bullet_text("Options to show map overlays only when in a party and/or only for the party leader.")
